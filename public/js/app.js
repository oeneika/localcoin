...
function stream_layers(a, b, c) {
    function d(a) {
        for (var c = 1 / (.1 + Math.random()), d = 2 * Math.random() - .5, e = 10 / (.1 + Math.random()), f = 0; f < b; f++) {
            var g = (f / b - d) * e;
            a[f] += c * Math.exp(-g * g)
        }
    }
    return arguments.length < 3 && (c = 0), d3.range(a).map(function () {
        var a, e = [];
        for (a = 0; a < b; a++) e[a] = c + c * Math.random();
        for (a = 0; a < 5; a++) d(e);
...
window.PJAX_ENABLED = !0, window.DEBUG = false;
var $lime = "#8CBF26",
    $red = "#f25118",
    $redDark = "#d04f4f",
    $blue = "#4e91ce",
...
chartsOff && (nv.addGraph = function () {}), COLOR_VALUES = [$red, $orange, $green, $blue, $teal, $redDark], window.colors = function () {
    return !!window.d3 && d3.scale.ordinal().range(COLOR_VALUES)
}(), $(function () {
    var a = $("#sidebar");
